...
namespace hwa_fgo{
    uwbprocesser::uwbprocesser(const baseprocesser& B, base_data* data) : baseprocesser(B, UWB_NODE), uwb_proc(_gset.get(), _name, data) {
    ...
    uwbprocesser::uwbprocesser(std::shared_ptr<set_base> gset, std::string site, base_log spdlog, base_data* data, base_time _beg, base_time _end) : baseprocesser(gset, spdlog, site, UWB_NODE, _beg, _end), uwb_proc(gset.get(), site, data)
    {
        make_dir("output\\range.txt");
        make_dir("output\\position.txt");
        make_dir("output\\pdop.txt");
