...
hwa_fgo::PseudorangeDDFactor::PseudorangeDDFactor(const base_time & cur_time, const std::pair<std::string, std::string> &base_rover_site, const base_allpar & params, const std::vector<std::pair<gnss_data_sats, gnss_data_sats>>& DD_sat_data, gnss_model_bias * bias_model, const std::pair<FREQ_SEQ, GOBSBAND> &freq_band):
...
void hwa_fgo::PseudorangeDDFactor::updatePara(base_allpar & params_tmp, const Eigen::Vector3d & Pi, const Eigen::Vector3d & Vi) const
{
	params_tmp = _params;
	int i = 0;
...
void hwa_fgo::PseudorangeDDFactor::trans2Eigen(const std::vector<std::vector<std::pair<int, double>>>& B, const std::vector<double>& P, const std::vector<double>& l, Eigen::Matrix<double, 2, 3>& B_new, Eigen::Matrix<double, 2, 2>& P_new, Eigen::Matrix<double, 2, 1> & l_new) const
{
	B_new.setZero();
	P_new.setZero();
	l_new.setZero();
...
bool hwa_fgo::PseudorangeDDFactor::Evaluate(double const * const * parameters, double * residuals, double ** jacobians) const
{
	...
	Eigen::Vector3d xyz_before = Eigen::Vector3d(xyz[0], xyz[1], xyz[2]);
...
void hwa_fgo::PseudorangeDDFactor::check(double ** parameters)
{
