...
int main(int argc, char** argv)
{
    ...
    auto gset = std::make_shared<set_fgo>();
    gset->app("hwa_fgo", "1.0.0", "$Rev: 2448 $", "(@whu,edu,cn)", __DATE__, __TIME__);
    ...
    for (size_t i = 0; i < inp.size() && itINP != inp.end(); ++i, ++itINP)
    {
        ...
        if (unique_coder) {
            ...
            if (ifmt != IFMT::IMU_INP && ifmt != IFMT::ODO_INP && ifmt != IFMT::UWBMSG_INP && ifmt != IFMT::LIDAR_INP)
            {
    ...
    std::map<int, std::unique_ptr<fgo_client>> vgipn;
    ...
    while (i < nsite) {
        ...
        vgipn[i] = std::make_unique<fgo_client>(site, site_base, beg, end, gset, my_logger, data);
