...
double hwa_vis::vis_feature::translation_threshold;            ///< transformation matrix threshold
double hwa_vis::vis_feature::huber_epsilon;                    ///< kernel function
double hwa_vis::vis_feature::estimation_precision;            ///< threshold of whether the update quantity is iterative
double hwa_vis::vis_feature::initial_damping;                ///< initial lambda of Levenberg-Marquart
int hwa_vis::vis_feature::outler_loop_max_iteration;        ///< iterations
int hwa_vis::vis_feature::inner_loop_max_iteration;            ///< iterations of depth
...
bool hwa_vis::vis_feature::triangulatePoint(const CamStateServer& cam_states) const
{
    ...
    Triple feature_direction(
        observations.begin()->second.position(0),
        observations.begin()->second.position(1), 1.0);
...
bool hwa_vis::vis_feature::checkMotion(const CamStateServer& cam_states) const
{
    ...
    for (auto iter = observations.begin(); iter != (observations.end()); iter++)
    {
        ...
        if (iter_2 == observations.end())
            break;
        x_vector.emplace_back(iter->second.position(0) - iter_2->second.position(0));
        y_vector.emplace_back(iter->second.position(1) - iter_2->second.position(1));
    }
...
bool hwa_vis::vis_feature::generateInitialGuess(
...
{    
    ...
    for (auto& m : observations)
    {
        ...
        measurements.push_back(m.second.position.head<2>());
        if(stereo)
        measurements.push_back(m.second.position.tail<2>());
    ...
    if(!generateInitialGuess(cam_poses[cam_poses.size() - 1], measurements[0],
    ...
    for (const auto& pose : cam_poses)
    {
        ...
        if (position(2) <= 0 || position(2) > 500)
        {
