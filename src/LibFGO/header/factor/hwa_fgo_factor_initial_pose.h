#ifndef HWA_FGO_INITIAL_POSE_FACTOR_H
#define HWA_FGO_INITIAL_POSE_FACTOR_H
	...
	*/
	...
	public:
		...
		virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
		{
			...
			//std::cout << "InitialPosBias residual: " << residual.transpose() << std::endl;
			...
			if (jacobians)
			{
				if (jacobians[0])
				{
					...
					jacobian_pose.block<3, 3>(3, 3) = base_att_trans::Qleft(init_Q.inverse() * Q).bottomRightCorner<3, 3>();
		...
		void check(double **parameters)
		{
			...
			for (int k = 0; k < 6; k++)
			{
				...
				if (a == 0)
					...
					Q = Q * base_att_trans::deltaQ(delta);
