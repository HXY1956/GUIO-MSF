...
vis_mono_vins_cpu::vis_mono_vins_cpu(set_base* _set, int cam_group_id) :vis_imgproc<cv::Mat>(_set, cam_group_id),
n_id(0)
{
...
PointCloud vis_mono_vins_cpu::ProcessBatch()
{
...
bool vis_mono_vins_cpu::Initialize()
{
...
bool vis_mono_vins_cpu::inBorder(const cv::Point2f& pt)
{
...
void vis_mono_vins_cpu::_reduceVector(vector<cv::Point2f>& v, vector<uchar> status)
{
    int j = 0;
...
void vis_mono_vins_cpu::_reduceVector(vector<int>& v, vector<uchar> status)
{
    int j = 0;
    for (int i = 0; i < int(v.size()); i++)
        if (status[i])
...
void vis_mono_vins_cpu::setMask()
{
    mask = cv::Mat(row, col, CV_8UC1, cv::Scalar(255));

    // prefer to keep features that are tracked for long time (VINS setMask)
    vector<pair<int, pair<cv::Point2f, pair<int, int>>>> cnt_pts_id;

    for (unsigned int i = 0; i < cur_pts.size(); i++)
        cnt_pts_id.push_back(make_pair(track_cnt[i], make_pair(cur_pts[i], make_pair(cur_ids[i], (int)i))));

    sort(cnt_pts_id.begin(), cnt_pts_id.end(), [](const pair<int, pair<cv::Point2f, pair<int, int>>>& a,
        const pair<int, pair<cv::Point2f, pair<int, int>>>& b)
        {
            return a.first > b.first;
        });

    vector<Eigen::Vector2d> kept_velocities;
    cur_pts.clear();
    cur_ids.clear();
    track_cnt.clear();
    ...
    for (auto& it : cnt_pts_id)
    {
        if (mask.at<uchar>(it.second.first) == 255)
        {
            ...
            cur_ids.push_back(it.second.second.first);
            track_cnt.push_back(it.first);
            kept_velocities.push_back(
                it.second.second.second < static_cast<int>(cur_velocities.size()) ?
                cur_velocities[it.second.second.second] : Eigen::Vector2d::Zero());

            cv::circle(mask, it.second.first, vins_min_dist, 0, -1);
        }
    }
    cur_velocities.swap(kept_velocities);
    after_setmask = cur_pts.size();
}

void vis_mono_vins_cpu::addnewFeatures()
{
    setMask();
    printf("[VINS-frontend] candidates: %d; track: %d; two_ransac: %d; cv_ransac: %d; ransac: %d/%d=%f; after_ransac: %d; mask: %d; max_cnt: %d; min_dist: %d\n",
        ...
        (static_cast<double>(before_tracking) + 1e-5), after_ransac, after_setmask, max_cnt, vins_min_dist);
    cout << "==================================" << endl;
    ...
    if (n_max_cnt > 0)
    {
        ...
        // VINS-Mono style: global budget + MIN_DIST spacing, no per-grid quota.
        cv::goodFeaturesToTrack(cur_img, n_pts, n_max_cnt, 0.01, vins_min_dist, mask);
    }
    else
        n_pts.clear();
    ...
    for (int i = 0; i < n_pts.size(); ++i)
    {
        ...
        cur_velocities.push_back(Eigen::Vector2d::Zero());
        ++added;
    }
    printf("[VINS-frontend] add new features: %d -> total %d\n",
        added, (int)cur_ids.size());
}
void vis_mono_vins_cpu::rejectWithF()
{
    if (cur_pts.size() >= 8)
    {
...
double vis_mono_vins_cpu::distance(cv::Point2f& pt1, cv::Point2f& pt2)
{
...
void vis_mono_vins_cpu::undistortPoints(
...
void vis_mono_vins_cpu::integrateImuData(cv::Matx33f& cam0_R_p_c)
{
...
void vis_mono_vins_cpu::predictFeatureTracking(
...
void vis_mono_vins_cpu::trackFeatures()
{
    Matx33f cam0_R_p_c;
    ...
    double dt = curr_img_msg.t - prev_img_msg.t;
    if (dt <= 1e-6)
        dt = 1e-6;
    cur_velocities.resize(cur_pts.size(), Eigen::Vector2d::Zero());
    for (int i = 0; i < cur_pts.size(); ++i)
    {
        cv::Point2f vel = (curr_cam0_points_undistorted[i] - prev_cam0_points_undistorted[i]) /
            static_cast<float>(dt);
        cur_velocities[i] = Eigen::Vector2d(vel.x, vel.y);
    }
}

void vis_mono_vins_cpu::rescalePoints(
...
void vis_mono_vins_cpu::twoPointRansac(
...
void vis_mono_vins_cpu::drawFeatures()
{
...
void vis_mono_vins_cpu::publish()
{
    ...
    if (cur_velocities.size() < cur_pts.size())
        cur_velocities.resize(cur_pts.size(), Eigen::Vector2d::Zero());
    ...
    for (int i = 0; i < static_cast<int>(cur_pts.size()); ++i)
    {
        ...
        feature_msg.velocity = cur_velocities[i];
