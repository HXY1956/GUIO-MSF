...
bool ProjectionFactor::Evaluate(double const* const* parameters, double* residuals, double** jacobians) const
{
    ...
    if (jacobians)
    {
        ...
        if (jacobians[0])
        {
            ...
            jaco_i.rightCols<3>() = ric.transpose() * Rj.transpose() * Ri * -base_att_trans::skewSymmetric(pts_imu_i);
        ...
        if (jacobians[1])
        {
            ...
            jaco_j.rightCols<3>() = ric.transpose() * base_att_trans::skewSymmetric(pts_imu_j);
        ...
        if (jacobians[2])
        {
            ...
            jaco_ex.rightCols<3>() = -tmp_r * base_att_trans::skewSymmetric(pts_camera_i) + base_att_trans::skewSymmetric(tmp_r * pts_camera_i) +
                base_att_trans::skewSymmetric(ric.transpose() * (Rj.transpose() * (Ri * tic + Pi - Pj) - tic));
...
void ProjectionFactor::check(double** parameters)
{
    ...
    for (int k = 0; k < 19; k++)
    {
        ...
        if (a == 0)
            ...
            Qi = Qi * base_att_trans::deltaQ(delta);
        else if (a == 2)
            Pj += delta;
        else if (a == 3)
            Qj = Qj * base_att_trans::deltaQ(delta);
        else if (a == 4)
            tic += delta;
        else if (a == 5)
            qic = qic * base_att_trans::deltaQ(delta);
        else if (a == 6)
            inv_dep_i += delta.x();
