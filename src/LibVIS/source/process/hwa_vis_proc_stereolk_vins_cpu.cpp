...
vis_stereo_vins_cpu::vis_stereo_vins_cpu(set_base* _set, int cam_group_id) :vis_imgproc<cv::Mat>(_set, cam_group_id),
...
IsInitialized(false)
{
    ...
    if (set != nullptr)
    {
        int tmp = set->min_dist(cam_group_id);
        if (tmp > 0)
            vins_min_dist = tmp;
    }
}

PointCloud vis_stereo_vins_cpu::ProcessBatch()
{
    ...
    if (equalize)
    {
        ...
        clahe->apply(cam0_curr_img, cam0_curr_img);
        if (!cam1_curr_img.empty())
            clahe->apply(cam1_curr_img, cam1_curr_img);
    }
    ...
    {
        if (_vecimu.size() != 0) {
            initializeFirstFrameVins();
            std::cout << "[VINS-frontend] detect first image" << std::endl;
            isFirstImg = false;
...
bool vis_stereo_vins_cpu::Initialize()
{
...
void vis_stereo_vins_cpu::createImagePyramids()
{
...
void vis_stereo_vins_cpu::initializeFirstFrame()
{
    // 获取当前摄像头 0 的图像
    ...
    for (auto& item : grid_new_features)
        std::sort(item.second.begin(), item.second.end(),
            &vis_stereo_vins_cpu::featureCompareByResponse);
...
void vis_stereo_vins_cpu::stereoMatch(const vector<cv::Point2f>& cam0_points,
...
void vis_stereo_vins_cpu::drawFeaturesStereo()
{
...
void vis_stereo_vins_cpu::publish()
{
...
void vis_stereo_vins_cpu::trackFeatures()
{
...
void vis_stereo_vins_cpu::integrateImuData(
...
void vis_stereo_vins_cpu::predictFeatureTracking(
...
void vis_stereo_vins_cpu::twoPointRansac(
...
void vis_stereo_vins_cpu::addNewFeatures()
{
    // 获取当前相机图像
    ...
    for (auto& item : new_feature_sieve)
    {
        ...
        {
            std::sort(item.begin(), item.end(), &vis_stereo_vins_cpu::keyPointCompareByResponse);
    ...
    // 对新特征点按响应值排序
    for (auto& item : grid_new_features)
        std::sort(item.second.begin(), item.second.end(), &vis_stereo_vins_cpu::featureCompareByResponse);
...
void vis_stereo_vins_cpu::pruneGridFeatures()
{
    // 遍历当前存储的特征点网格
    for (auto& item : *curr_features_ptr)
    {
        ...
        std::sort(grid_features.begin(), grid_features.end(),
            &vis_stereo_vins_cpu::featureCompareByLifetime);
...
vector<cv::Point2f> vis_stereo_vins_cpu::distortPoints(
...
void vis_stereo_vins_cpu::undistortPoints(
...
bool vis_stereo_vins_cpu::featureCompareByResponse(
...
template <typename T>
void vis_stereo_vins_cpu::removeUnmarkedElements(
...
void vis_stereo_vins_cpu::rescalePoints(
...
bool vis_stereo_vins_cpu::keyPointCompareByResponse(
...
bool vis_stereo_vins_cpu::featureCompareByLifetime(
    const FeaturePoint& f1,
    const FeaturePoint& f2)
{
...
vector<Triple> vis_stereo_vins_cpu::stereoSemiDenseMatch(cv::Mat& disparity)
{
    vector<Triple> pl;
...
void vis_stereo_vins_cpu::calculateIPM(cv::Mat& ipm)
{
...
// ---------------------------------------------------------------------------
// VINS-Mono style feature management (used by vis_stereo_vins_cpu)
// ---------------------------------------------------------------------------

void vis_stereo_vins_cpu::setMaskVins()
{
    const int rows = cam0_curr_img.rows;
    const int cols = cam0_curr_img.cols;
    if (rows <= 0 || cols <= 0)
        return;
    ...
    for (const auto& det : dyna_box)
        cv::rectangle(mask, det.box, cv::Scalar(0), cv::FILLED);

    // Prefer long-living features (VINS setMask sorts by track_cnt).
    vector<pair<int, FeaturePoint>> cnt_pts;
    for (const auto& item : *curr_features_ptr)
        for (const auto& f : item.second)
            cnt_pts.push_back(make_pair(f.lifetime, f));

    sort(cnt_pts.begin(), cnt_pts.end(), [](const pair<int, FeaturePoint>& a,
        const pair<int, FeaturePoint>& b)
        {
            return a.first > b.first;
        });

    const int cell_rows = std::max(1, grid_row);
    const int cell_cols = std::max(1, grid_col);
    const int cell_h = std::max(1, rows / cell_rows);
    const int cell_w = std::max(1, cols / cell_cols);

    GridFeatures filtered;
    for (int code = 0; code < cell_rows * cell_cols; ++code)
        filtered[code] = vector<FeaturePoint>();

    int kept = 0;
    for (auto& it : cnt_pts)
    {
        const cv::Point2f& pt = it.second.cam0_point;
        int ix = cvRound(pt.x);
        int iy = cvRound(pt.y);
        if (ix < 0 || ix >= cols || iy < 0 || iy >= rows)
            continue;
        if (mask.at<uchar>(iy, ix) == 255)
        {
            int r = std::min(iy / cell_h, cell_rows - 1);
            int c = std::min(ix / cell_w, cell_cols - 1);
            filtered[r * cell_cols + c].push_back(it.second);
            cv::circle(mask, pt, vins_min_dist, 0, -1);
            ++kept;
        }
    }
    *curr_features_ptr = filtered;
    after_setmask = kept;
}

void vis_stereo_vins_cpu::initializeFirstFrameVins()
{
    const Mat& img = cam0_curr_img;
    if (img.empty())
        return;

    Mat det_mask(img.rows, img.cols, CV_8UC1, Scalar(255));
    vector<cv::Point2f> cam0_points;
    cv::goodFeaturesToTrack(img, cam0_points, max_cnt, 0.01, vins_min_dist, det_mask);

    vector<cv::Point2f> cam1_points;
    vector<unsigned char> inlier_markers;
    if (!cam0_points.empty())
        stereoMatch(cam0_points, cam1_points, inlier_markers);

    const int cell_rows = std::max(1, grid_row);
    const int cell_cols = std::max(1, grid_col);
    const int cell_h = std::max(1, img.rows / cell_rows);
    const int cell_w = std::max(1, img.cols / cell_cols);
    ...
    int inserted = 0;
    for (int i = 0; i < static_cast<int>(cam0_points.size()); ++i)
    {
        if (i >= static_cast<int>(inlier_markers.size()) || inlier_markers[i] == 0)
            continue;
        const cv::Point2f& p0 = cam0_points[i];
        const cv::Point2f& p1 = cam1_points[i];
        int r = std::min(static_cast<int>(p0.y / cell_h), cell_rows - 1);
        int c = std::min(static_cast<int>(p0.x / cell_w), cell_cols - 1);
        ...
        f.cam0_point = p0;
        f.cam1_point = p1;
        f.velocity = Eigen::Vector2d::Zero();
        grid_new_features[r * cell_cols + c].push_back(f);
        ++inserted;
    }
    *curr_features_ptr = grid_new_features;

    printf("[VINS-frontend] first frame: detected %d, stereo matched %d, max_cnt %d, min_dist %d\n",
        static_cast<int>(cam0_points.size()), inserted, max_cnt, vins_min_dist);
}

void vis_stereo_vins_cpu::addNewFeaturesVins()
{
    setMaskVins();

    int kept_num = 0;
    for (const auto& item : *curr_features_ptr)
        kept_num += static_cast<int>(item.second.size());

    int n_max_cnt = max_cnt - kept_num;
    if (n_max_cnt <= 0)
    {
        new_added_num = 0;
        printf("[VINS-frontend] tracked %d >= max_cnt %d, no new features\n",
            kept_num, max_cnt);
        return;
    }

    vector<cv::Point2f> cam0_points;
    cv::goodFeaturesToTrack(cam0_curr_img, cam0_points, n_max_cnt, 0.01,
        vins_min_dist, mask);
    ...
    const int cell_rows = std::max(1, grid_row);
    const int cell_cols = std::max(1, grid_col);
    const int cell_h = std::max(1, cam0_curr_img.rows / cell_rows);
    const int cell_w = std::max(1, cam0_curr_img.cols / cell_cols);
    ...
    for (int i = 0; i < static_cast<int>(cam0_points.size()); ++i)
    {
        ...
        f.cam1_point = p1;
        f.velocity = Eigen::Vector2d::Zero();
        (*curr_features_ptr)[r * cell_cols + c].push_back(f);
        ++inserted;
    }
    new_added_num = inserted;

    printf("[VINS-frontend] tracked %d, stereo matched new %d -> total %d (max_cnt %d, min_dist %d)\n",
        kept_num, inserted, kept_num + inserted, max_cnt, vins_min_dist);
}
