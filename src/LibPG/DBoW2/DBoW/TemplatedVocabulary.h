...
void TemplatedVocabulary<TDescriptor,F>::loadBin(const std::string &filename) {
  ...
  // MUST open in binary mode on Windows: text mode treats 0x1A (Ctrl-Z) as EOF
  // and translates CRLF, truncating the node array at the first 0x1A byte and
  // leaving the whole tree uninitialized (root gets every node as a child and
  // transform() loops forever). VINS only works because Linux text mode is
  // equivalent to binary mode.
  ...
  for(unsigned int i = 0; i < voc.nNodes; ++i)
  {
    ...
    // Windows: the default boost Block is unsigned long (32 bits), so 4x uint64
    // would build a 128-bit bitset while query descriptors are 256-bit; the
    // resulting (a ^ b) in FBrief::distance would read out of bounds (the size
    // assert is compiled out in release). Split each uint64 into two 32-bit
    // blocks so vocabulary descriptors are 256 bits, matching DVision::BRIEF.
    const uint64_t* d64 = voc.nodes[i].descriptor;
    unsigned long d32[8];
    for (int j = 0; j < 4; ++j) {
        d32[2 * j]     = (unsigned long)(d64[j] & 0xFFFFFFFFULL);
        d32[2 * j + 1] = (unsigned long)(d64[j] >> 32);
    }
    m_nodes[nid].descriptor = boost::dynamic_bitset<>(d32, d32 + 8);
