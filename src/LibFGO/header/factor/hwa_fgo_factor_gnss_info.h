#ifndef HWA_FGO_GNSS_INFO_FACTOR
#define HWA_FGO_GNSS_INFO_FACTOR
...
namespace hwa_fgo
{
	...
	public:
		 ...
		 */
		void addResidualBlockInfo(GNSSResidualBlockInfo *residual_block_info, const std::map<long, std::vector<int>> &para_ids);	
		 ...
		 */
		void constructEqu(Matrix variance);
		 ...
		 */
		void constructEqu(Matrix variance, int var_type); //var_type: 0-> initial variance; 1 posterior variance
		 ...
		 */
		void constructEqu_fromCeres(Matrix variance);
		...
		//gnss_amb_manager* amb_manager = nullptr;  // 指向外部模糊度管理器
		int m = 0, n = 0;			
		...
		Matrix weight;
		Vector v_norm;
		Matrix Qx;               /// parameter variance after optimization
		Matrix Qx0;              /// initial variance
	...
	class MarginalizationGNSSFactor : public ceres::CostFunction
	{
	public:
